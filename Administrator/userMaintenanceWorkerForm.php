...
if (isset($_POST['sumbit'])) {
    ...
    $stmt_user="SELECT * from adminmaintenanceworkeraccount where m_workerEmail='$email'" ;

    $result=$connect->query($stmt_user);
    if ($result->num_rows>0) {
        echo "email already exist";
        header('Location:userMaintenanceWorkerForm.php');
        // Administrator\userMaintenanceWorkerForm.php
    } else {
        $sql="INSERT INTO adminmaintenanceworkeraccount (m_workerName,m_workerEmail,m_workerMobile,password) VALUES('$name','$email','$mobile','$password' )";
        $result=$connect->query($sql);
        if ($result) {
            // echo "Data inserted Successfully";
            header('Location:userMaintenance.php');
        //  Administrator\userMaintenance.php
        } else {
